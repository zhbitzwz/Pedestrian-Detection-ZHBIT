...
int main()
{
	...
	if(TRAIN)
	{
		...
		ifstream finPos("D:\\opencv\\opencv2.4.11\\opencv\\test3\\pos\\pog.txt");//正样本图片的文件名列表
		//ifstream finPos("PersonFromVOC2012List.txt");//正样本图片的文件名列表
		ifstream finNeg("D:\\opencv\\opencv2.4.11\\opencv\\test3\\neg\\neg.txt");//负样本图片的文件名列表
		...
		for(int num=0; num<PosSamNO && getline(finPos,ImgName); num++)
		{
			...
			ImgName = "D:\\opencv\\opencv2.4.11\\opencv\\test3\\pos\\" + ImgName;//加上正样本的路径名
		...
		{
			cout<<"处理："<<ImgName<<endl;
			ImgName = "D:\\opencv\\opencv2.4.11\\opencv\\test3\\neg\\" + ImgName;//加上负样本的路径名
			Mat src = imread(ImgName);//读取图片
	...
	***************************************************************************************************/
	...
	/**************读入图片进行HOG行人检测******************/
	Mat src = imread("D:\\opencv\\opencv2.4.11\\opencv\\test3\\1.png");
	...
	while(1)
	{
			...
			Mat frame2 = Mat(dsize,CV_32S);
			resize(frame,frame2,dsize);


			vector<Rect> found, found_filtered;//矩形框数组
			cout<<"进行多尺度HOG人体检测"<<endl;
			myHOG.detectMultiScale(frame2, found, 0, Size(8,8), Size(32,32), 1.05, 2);//对图片进行多尺度行人检测
			...
			for(int i=0; i<found_filtered.size(); i++)
			{
				...
				rectangle(frame2, r.tl(), r.br(), Scalar(0,255,0), 3);
			}


			namedWindow("src");
			imshow("src",frame2);
			if(waitKey(33)==27) break;//注意：imshow之后必须加waitKey，否则无法显示图像
	}
